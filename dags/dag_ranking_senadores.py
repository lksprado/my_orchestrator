...
from include.local_setup.src.pipelines.legislativo.ranking_politicos.ranking_senadores import (
    extract as extract_ranking_senadores,
)
from include.local_setup.src.pipelines.legislativo.ranking_politicos.ranking_senadores import (
    transform as transform_ranking_senadores,
)
...
def ranking_senadores_pipeline():
    logger = logging.getLogger("DAG: ranking_senadores")

    cfg = PipelineConfig(
        **load_source_config(_CONFIG_FILE, source="senadores", env="airflow")
    )
    target = cfg.db_table
    ...
    etl = GenericETL(
        cfg=cfg, extract_fn=extract_ranking_senadores, load_fn=None, log=logger
    )
    ...
    @task
    def t_transform():
        transform_ranking_senadores(cfg)
...
dag = ranking_senadores_pipeline()
