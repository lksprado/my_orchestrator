...
from include.local_setup.src.pipelines.legislativo.senado.senado_senadores import (
    extract as extract_senadores,
    transform as transform_senadores,
)
...
logger = logging.getLogger("DAG: senadores")
...
def senadores_pipeline():
    cfg = PipelineConfig(**load_source_config(_CONFIG_FILE, source="senadores", env="airflow"))
    target = cfg.db_table
    ...
    etl = GenericETL(cfg=cfg, extract_fn=extract_senadores, load_fn=None, log=logger)
