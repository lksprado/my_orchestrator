...
from include.local_setup.src.pipelines.legislativo.ecidadania.ecidadania_mais_votados import (
    extract as extraction_mais_votados,
    transform as transform_mais_votados,
)
...
def maisvotados_pipeline():
    cfg = PipelineConfig(**load_source_config(_CONFIG_FILE, source="mais_votados", env="airflow"))
    target = cfg.db_table
    ...
    etl = GenericETL(cfg=cfg, extract_fn=extraction_mais_votados, load_fn=None, log=logger)
