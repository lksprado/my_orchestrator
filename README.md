...
O workflow faz checkout dos três repos e roda `deploy/build_prod.sh` (só as DAGs de `deploy/prod-dags.txt`, os arquivos de `deploy/prod/` no lugar dos de dev, `my_ingestion` `src/` e `my_analytics`, `DEPLOYED.txt`, `rsync --delete` para `/srv/airflow`). Como em dev, os containers leem do disco as DAGs, o `my_ingestion` e o projeto dbt, então **o deploy normal é só rsync, sem restart**. O `deploy/estado.sh` compara hashes com o último deploy: `/srv/airflow/start.sh restart` (rebuild) só roda quando mudam `Dockerfile`, `requirements.txt`, `packages.txt`, `deploy/prod/*`, `plugins/` ou o `.env`, e `dbt deps` (dentro do scheduler) só quando muda o `package-lock.yml`. Por fim, `deploy/verify_prod.sh` confere 0 import errors, número de DAGs igual à allowlist e nada em `0.0.0.0`. Para aplicar um `.env` alterado ou forçar restart: *Actions → Deploy prod → Run workflow*. Mudanças só em `.md` não disparam deploy.
