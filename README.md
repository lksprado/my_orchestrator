...
| `investimentos_fgc` | 2026-09-14 | seed_de_para_instituicoes_fgc.csv no the_dw | falhou: o SQL do my_ingestion lê intermediate.int_renda_fixa, e o the_dw gera intermediate_financas.int_renda_fixa. Nenhum seed gravado. Entrada instituicoes_conglomerado_prudencial.csv copiada de ~/workspace/investments para o lake | ❌ não passou (desalinhamento com o the_dw) |
| `atacadao_historico` | 2026-09-14 | seed_minha_inflacao.csv no the_dw: 1271 linhas, colunas ... Categoria, Mes | execução ok e 1271 linhas, mas o arquivo gerado (minha_inflacao.csv) tem Mês passado e Var no lugar de Categoria e Mes; arquivo removido do the_dw. Entradas copiadas de ~/workspace/webscraping-inflation/data/months para bronze/inflation/months | ❌ não passou (formato diverge do seed do the_dw) |
...
| `dag_dbt_the_dw` | 2026-09-13, revalidado 2026-09-14 | — | 238 de 244 tasks, igual nas duas execuções. Na de 14/09 (the_dw `b7f8248`, raw alinhada e recarregada) todos os models de staging de Câmara, Senado, e-Cidadania, Radar, Ranking, Vide, Google Sheets e APsystems rodaram. Três erros no `the_dw`, reproduzidos fora do Airflow em `94f1b44`: `stg_proventos` faz `quantidade::INT` com valores `"75.0"`; o teste `not_null_fct_products_sku` procura uma coluna que a view não tem; o teste de `fct_games.is_regulation_loss` acha 3 nulos | ❌ não passou (conteúdo do the_dw) |
| exportações | 2026-09-14 | — | `extract_postgres_demodados` e `extract_postgres_mydatawarehouse` gravaram os CSVs em `gold/` | ✅ passou |
...
Contagem do dbt: o `the_dw` tem 459 nós (162 models, 223 testes, 58 sources, 16 seeds) e o Cosmos gera 244 tasks, sem perder nada. Cada model vira uma task de run; os 66 models com testes ganham uma task `.test` que roda todos os testes dele; as seeds viram tasks; sources não viram task.
...
| DAG | O que faz | Schedule |
|-----|-----------|----------|
| `dag_dbt_the_dw` | dbt do `the_dw` completo via Cosmos | manual (09:30 depois de validado) |
| `extract_postgres_mydatawarehouse` | Exporta marts de energia e inflação para CSV em `gold/` | diário 03:30 |
| `smoke_my_ingestion` | Smoke test do wiring (imports, `.env`, banco, the_dw) | manual |
