...
- `deploy/versions.txt` — exact `my_ingestion`/`my_analytics` commits prod runs; promoting = bumping a SHA in its own PR (`main` is protected, every change goes through a PR with manual approval)
