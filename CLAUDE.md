...
Apache Airflow orchestration project (repo `my_orchestrator`) on Astro Runtime 3.0-10 (Airflow 3.0.6, Python 3.12). It only orchestrates: ingestion code lives in the `my_ingestion` monorepo and transformation in the `my_analytics` dbt project. Neither is a submodule: dev mounts `~/workspace` by volume, prod bakes the commits pinned in `deploy/versions.txt` into the image. Data follows a medallion architecture: landing/bronze files in the lake → `raw_<fonte>.*` tables → dbt staging/intermediate/marts. Code, comments and docs are in Portuguese.

Two environments, selected by environment variables only: `dev` = this machine (CLI of my_ingestion and this local Airflow, database `analytics_dev`) and `prod` = the `atb` server (`/srv/airflow`, built by `deploy/deploy_prod.sh`). Same DAG code in both; only the `.env` changes.
...
deploy/deploy_prod.sh [--dry-run]         # prod: build from origin/main + allowlist + pinned SHAs, rsync to atb:/srv/airflow
ssh -t atb /srv/airflow/start.sh restart   # prod: rebuild/restart (asks for sudo)
```
...
- `deploy/versions.txt` — exact `my_ingestion`/`my_analytics` commits prod runs; promoting = bumping a SHA in its own commit
- `deploy/prod/` — prod-only files swapped in by `deploy/deploy_prod.sh`: compose override, `.astro/config.yaml`, `start.sh`, `.env.example`
...
Astro project at `/srv/airflow` on `atb`, generated by `deploy/deploy_prod.sh` run on **this** machine (my_ingestion is private and atb holds no GitHub credential). The script refuses a dirty tree or `HEAD != origin/main`, `git archive`s this repo, keeps only the DAGs in `deploy/prod-dags.txt`, swaps in `deploy/prod/*`, extracts my_ingestion `src/` and my_analytics at the `versions.txt` SHAs into the image paths (plus `dbt deps` from `package-lock.yml`, since `dbt_packages/` is not versioned), writes `DEPLOYED.txt` and `rsync --delete`s to atb (never touching `.env` nor the auth passwords file). Then `ssh -t atb /srv/airflow/start.sh restart` (sudo; `lcs` is not in the docker group).

- Same image as dev (Runtime 3.0-10, Python 3.12); code is baked in, no bind mounts except the lake.
...
- Lake = SeaweedFS buckets FUSE-mounted at `/srv/lake` by the homelab `seaweedfs-mount` service, bound to `/usr/local/airflow/mylake`. `start.sh` refuses to start if it is not mounted.
