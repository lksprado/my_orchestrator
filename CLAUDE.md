...
### Replicação da raw de prod para dev (`raw_pull_prod`)

`dags/dag_raw_pull_prod.py` roda `include/my_ingestion/scripts/raw_copy.sh pull` de hora em hora e copia os 14 schemas `raw_*` de `analytics_prod` para o `analytics_dev`, para a raw que o dbt de dev consome não envelhecer (ciclo em regime: ~55 s). O script decide por tabela entre cópia completa e delta (`loaded_at_utc` > o máximo do destino), nunca dá `DROP` (as views do dbt sobrevivem) e recebe as credenciais das `DB__DEV__*`/`DB__PROD__*` do ambiente — o `.env` do my_ingestion não existe no container (my_ingestion `>= PR #14`). Desde my_ingestion `PR #15` cada tabela é **atômica**: a origem escreve num arquivo e o destino carrega numa transação só, com conferência do número de linhas, então falha nenhuma deixa tabela vazia, parcial ou com buraco no delta.

A DAG é **só de dev** e por isso `dags/dag_raw_pull_prod.py` está no `.gitignore` (em prod não existiria destino: o `analytics_dev` é local), então o deploy nunca a leva para o atb; a primeira task ainda pula fora de `ENV=dev` como cinto de segurança e barra o caso de **coluna que existe em prod e falta em dev** (a lista de colunas vem da origem, então essas tabelas dariam `ROLLBACK` a cada execução; crie com `ALTER TABLE ... ADD COLUMN`). Tabela que só existe em prod, essa o script cria sozinho com `pg_dump -s`: a imagem traz `postgresql-client-16` (ver `Dockerfile`), porque o cliente padrão do bookworm é o 15 e **os dois servidores, dev e prod, são 16** — `pg_dump` se recusa a ler servidor mais novo.
