...
`dags/dag_raw_pull_prod.py` roda `include/my_ingestion/scripts/raw_copy.sh pull` de hora em hora e copia os 14 schemas `raw_*` de `analytics_prod` para o `analytics_dev`, para a raw que o dbt de dev consome não envelhecer. O script decide por tabela entre cópia completa e delta (`loaded_at_utc` > o máximo do destino), é `TRUNCATE` + `COPY` (nunca `DROP`, as views do dbt sobrevivem) e recebe as credenciais das `DB__DEV__*`/`DB__PROD__*` do ambiente — o `.env` do my_ingestion não existe no container (my_ingestion `>= PR #14`). A DAG é **só de dev** e por isso `dags/dag_raw_pull_prod.py` está no `.gitignore` (em prod não existiria destino: o `analytics_dev` é local), então o deploy nunca a leva para o atb; a primeira task ainda pula fora de `ENV=dev` como cinto de segurança. Ela também barra antes de copiar dois casos que quebrariam no meio:

- **tabela nova em prod**: o script a criaria com `pg_dump -s`, mas o cliente da imagem é 15 e o servidor de prod é 16 (`server version mismatch`). Rode o `pull` desse schema uma vez do host, que tem cliente 16.
- **coluna que existe em prod e falta em dev**: a lista de colunas vem da origem, então o `COPY` quebraria com a tabela de dev já truncada. Crie com `ALTER TABLE ... ADD COLUMN`.
